#!/usr/bin/env python3
"""Reproducibility of LLC net-turning structure across SSL seeds.

Purpose
-------
Test whether the visually observed LLC turning differences are reproducible
across independently trained SSL encoders and held-out VALIDATION fish.

Primary question
----------------
Within the conventional LLC bout class, do aligned SSL subclusters show
consistent differences in ``turn_net_rad`` across seeds 11, 23, 37, 51, 79?

Design
------
- Restrict analysis to known class LLC only.
- Use the selected SSL k=8 labels.
- Use seed 11 as the reference cluster numbering.
- Estimate Hungarian mappings from TRAIN assignments only.
- Apply the same TRAIN-derived mappings unchanged to VALIDATION.
- Analyze TRAIN + VALIDATION only.
...
- per-aligned-subcluster:
    ...
    p25 / p75
- eta-squared for:
    turn_net_rad ~ aligned SSL subcluster
- standardized range of subcluster means

Across seeds:
- pairwise Spearman correlation between the 8-element aligned-subcluster
  mean-turn profiles
- pairwise Spearman correlation between median-turn profiles
- per-cluster directional consistency across seeds
- TRAIN-to-VALIDATION profile correlation for each seed

Directional categories
----------------------
A subcluster's mean turn is classified as:
- positive if mean > ``--neutral-threshold``
- negative if mean < -``--neutral-threshold``
- neutral otherwise

The default threshold is 0.10 radians. This is a descriptive interpretation
threshold, not a hypothesis-testing cutoff.

Important guardrails
--------------------
- This analysis tests reproducibility of a kinematic association.
- It does NOT establish a novel biological behavior.
- Cluster ID alignment is learned only from TRAIN.
- High correlation of turn profiles means that the same aligned SSL regions
  tend to carry similar turning biases across seeds.
- A low effect in VALIDATION would weaken the seed-11 visual interpretation.

Outputs
-------
data/processed/DS-005/ssl_llc_turn_reproducibility/
    seed11/
        train.json
        validation.json
    seed23/
    ...
    summary.json
    LLC_TURN_REPRODUCIBILITY_SHA256SUMS

Usage
-----
From repository root:

    PYTHONPATH=. python3 src/validation/ssl_llc_turn_reproducibility.py
...
def prohibit_test_path(path: Path) -> None:
    if "test" in str(path).lower():
        raise RuntimeError(
            f"TEST access prohibited during LLC turn reproducibility: {path}"
        )
...
def detect_label_column(
    ...
    if requested is not None:
        if requested not in available:
            raise RuntimeError(
                f"Requested label column {requested!r} is absent. "
...
def load_baseline_raw(
    ...
    *,
    ...
    if X.shape != expected:
        raise RuntimeError(
            f"{path}: expected shape {expected}, got {X.shape}."
        )

    if TURN_FEATURE not in feature_names:
        raise RuntimeError(
            f"{TURN_FEATURE!r} is not present in frozen baseline features. "
            f"Available features: {feature_names}"
        )

    if not np.isfinite(X).all():
        raise RuntimeError(
            f"{path}: non-finite values in handcrafted features."
        )
    ...
    *,
    ...
    with path.open(
        ...
        if reader.fieldnames is None:
            raise RuntimeError(f"{path}: CSV header missing.")
        ...
        for expected_row, row in enumerate(reader):
            ...
            fields["fish_id"].append(row["fish_id"])
            fields["session_id"].append(row["session_id"])
            fields["bout_index"].append(str(int(row["bout_index"])))
            fields["partition"].append(row["partition"])
            fields["context_id"].append(row["context_id"])
            fields["bout_id"].append(row["bout_id"])
            fields["known_label"].append(
                normalize_known_label(
                    row[detected_label_column]
                )
            )

    if len(fields["fish_id"]) != EXPECTED_ROWS[partition]:
        raise RuntimeError(
...
def verify_alignment(
    ...
    *,
    ...
    for field in (
        ...
        if a.shape != b.shape:
            raise RuntimeError(
                f"Alignment shape mismatch seed={seed}, "
...
def load_ssl_labels(
    ...
    *,
    ...
    if not np.array_equal(
        ...
        raise RuntimeError(
            f"{path}: expected labels 0..{EXPECTED_SSL_K - 1}."
        )
...
def eta_squared(
    ...
    for cluster in np.unique(labels):
        ...
        cluster_mean = float(np.mean(subset))

        ss_between += (
            float(subset.size)
            * (cluster_mean - grand_mean) ** 2
        )
...
def standardized_mean_range(
    ...
    if values.size == 0 or global_std <= 0:
        return 0.0

    means = [
        float(np.mean(values[labels == cluster]))
...
def characterize_llc_turn(
    *,
    ...
    if values.shape != labels.shape:
        raise RuntimeError(
            f"Value/label shape mismatch: {values.shape} vs {labels.shape}"
        )
    ...
    for cluster in range(EXPECTED_SSL_K):
        ...
        mean_profile[cluster] = mean
        median_profile[cluster] = median

        per_cluster[str(cluster)] = {
            "count": int(subset.size),
            "fraction": float(subset.size / total),
            "mean": mean,
            "median": median,
    ...
    return {
        ...
        "median_profile": median_profile.tolist(),
...
def safe_spearman(
    ...
    return rho, pvalue, int(x.size)


def pairwise_profile_correlations(
    per_seed_results: Mapping[int, Dict[str, Any]],
    *,
    profile_key: str,
) -> Dict[str, Any]:
    ...
    for i, seed_a in enumerate(seeds):
        for seed_b in seeds[i + 1:]:
            rho, pvalue, n = safe_spearman(
                per_seed_results[seed_a][profile_key],
                per_seed_results[seed_b][profile_key],
            )
    ...
    return {
        ...
        "finite_pair_count": int(finite_rhos.size),
        "mean_rho": (
            float(np.mean(finite_rhos))
            if finite_rhos.size
            else None
        ),
        "median_rho": (
            float(np.median(finite_rhos))
            if finite_rhos.size
            else None
        ),
        "min_rho": (
            float(np.min(finite_rhos))
            if finite_rhos.size
            else None
        ),
        "max_rho": (
            float(np.max(finite_rhos))
            if finite_rhos.size
            else None
        ),
        "pairs": pairs,
...
def directional_consistency(
    ...
    for cluster in range(EXPECTED_SSL_K):
        directions = [
            per_seed_results[seed]["per_cluster"][str(cluster)]["direction"]
            for seed in seeds
        ]

        counts = {
            "positive": int(directions.count("positive")),
            "negative": int(directions.count("negative")),
            "neutral": int(directions.count("neutral")),
            "missing": int(directions.count("missing")),
        }
        ...
        if nonmissing:
            mode = max(
                ...
                key=lambda value: nonmissing.count(value),
            )
            consistency = float(
                nonmissing.count(mode)
                / len(nonmissing)
            )
        else:
            mode = "missing"
        ...
        output[str(cluster)] = {
            "directions_by_seed": {
                str(seed): directions[i]
                for i, seed in enumerate(seeds)
            },
            "counts": counts,
            "modal_direction": mode,
            "modal_consistency_fraction": consistency,
        }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Quantify reproducibility of LLC turn_net_rad differences "
            "across frozen SSL seeds."
        )
    )
    ...
    parser.add_argument(
        ...
        default=0.10,
        help=(
            "Absolute mean turn_net_rad <= threshold is labeled neutral. "
            "Default: 0.10 rad."
        ),
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if args.neutral_threshold < 0:
        raise ValueError("--neutral-threshold must be >= 0.")
    ...
    if REFERENCE_SEED not in seeds:
        raise RuntimeError(
            f"Reference seed {REFERENCE_SEED} is absent from frozen seeds."
        )
    ...
    print("=" * 80)
    print("DS-005 LLC TURN REPRODUCIBILITY")
    print("=" * 80)
    print(f"Known class:       {KNOWN_CLASS}")
    print(f"Feature:           {TURN_FEATURE}")
    print(f"SSL seeds:         {list(seeds)}")
    print(f"Reference seed:    {REFERENCE_SEED}")
    print(
        f"Neutral threshold: +/-{args.neutral_threshold:.3f} rad"
    )
    print(
    ...
    turn_index = feature_names.index(
        TURN_FEATURE
    )

    metadata_by_seed: Dict[int, Dict[str, Dict[str, np.ndarray]]] = {}
    labels_by_seed: Dict[int, Dict[str, np.ndarray]] = {}
    ...
    for seed in seeds:
        ...
        for partition in PARTITIONS:
            ...
            labels_by_seed[seed][partition] = labels
    ...
    for seed in seeds:
        ...
        mappings[seed] = hungarian_map_to_reference(
            train_reference,
            labels_by_seed[seed]["train"],
        )

    aligned_labels: Dict[int, Dict[str, np.ndarray]] = {
        seed: {
            partition: apply_mapping(
                labels_by_seed[seed][partition],
                mappings[seed],
            )
            for partition in PARTITIONS
        }
    ...
    for partition in PARTITIONS:
        if not np.any(llc_masks[partition]):
            raise RuntimeError(
                f"No {KNOWN_CLASS} bouts found in {partition}."
            )
    ...
    for seed in seeds:
        ...
        for partition in PARTITIONS:
            mask = llc_masks[partition]

            turn_values = baseline_x[
                partition
            ][
                mask,
                turn_index,
            ...
            result = characterize_llc_turn(
                values=turn_values,
                labels=llc_labels,
                neutral_threshold=args.neutral_threshold,
            )

            per_seed[seed][partition] = result
            ...
            print(
                ...
                f"mean-range-z={result['standardized_mean_range']:.3f}"
            )

            direction_text = ", ".join(
                f"{cluster}:{data['direction']}"
                for cluster, data
                in result["per_cluster"].items()
            )
        ...
        train_val_rho, train_val_p, train_val_n = safe_spearman(
            per_seed[seed]["train"]["mean_profile"],
            per_seed[seed]["validation"]["mean_profile"],
        )

        per_seed[seed]["train_validation_mean_profile_spearman"] = {
            "rho": train_val_rho,
            "pvalue": train_val_p,
            "clusters_compared": train_val_n,
        }

        print(
            "  TRAIN-vs-VALIDATION mean-profile Spearman: "
            f"{train_val_rho if train_val_rho is not None else 'NA'}"
        )
    ...
    for partition in PARTITIONS:
        partition_seed_results = {
            seed: per_seed[seed][partition]
            for seed in seeds
        }

        cross_seed[partition] = {
            "eta_squared": summarize_scalar_metric(
                partition_seed_results,
                "eta_squared",
            ),
            "standardized_mean_range": summarize_scalar_metric(
                partition_seed_results,
                "standardized_mean_range",
            ),
            "mean_profile_spearman": pairwise_profile_correlations(
                partition_seed_results,
                profile_key="mean_profile",
            ),
            "median_profile_spearman": pairwise_profile_correlations(
                partition_seed_results,
    ...
    train_validation_rhos = np.asarray(
        [
            per_seed[seed][
                "train_validation_mean_profile_spearman"
            ]["rho"]
            for seed in seeds
            if per_seed[seed][
                "train_validation_mean_profile_spearman"
    ...
    cross_seed["train_validation_generalization"] = {
        "mean_profile_spearman_by_seed": {
        ...
        "mean_rho": (
            float(np.mean(train_validation_rhos))
            if train_validation_rhos.size
            else None
        ),
        "min_rho": (
            float(np.min(train_validation_rhos))
            if train_validation_rhos.size
            else None
        ),
        "max_rho": (
            float(np.max(train_validation_rhos))
            if train_validation_rhos.size
            else None
        ),
    }
    ...
    validation_eta = cross_seed[
        "validation"
    ]["eta_squared"]

    validation_mean_rho = cross_seed[
        "validation"
    ][
        "mean_profile_spearman"
    ]["mean_rho"]

    validation_median_rho = cross_seed[
        "validation"
    ][
    ...
    summary = {
        "dataset_id": "DS-005",
        "analysis": "ssl_llc_turn_reproducibility",
        "known_class": KNOWN_CLASS,
        "feature": TURN_FEATURE,
        "ssl_training_seeds": list(seeds),
        "reference_seed": REFERENCE_SEED,
        ...
        "validation_mean_eta_squared": validation_eta["mean"],
        "validation_eta_squared_range": {
            "min": validation_eta["min"],
            "max": validation_eta["max"],
        },
        "validation_mean_pairwise_mean_profile_spearman": (
            validation_mean_rho
        ),
        "validation_mean_pairwise_median_profile_spearman": (
            validation_median_rho
        ),
        "mean_train_validation_profile_spearman": (
            cross_seed[
                "train_validation_generalization"
            ]["mean_rho"]
        ),
        "validation_directional_consistency": (
            cross_seed[
                "validation"
        ...
        "interpretation_guardrails": {
            ...
            "turn_association_not_proof_of_new_behavior": True,
            "test_partition_used": False,
        },
        "test_partition_used": False,
    }
    ...
    print("=" * 80)
    print("LLC TURN REPRODUCIBILITY SUMMARY")
    print("=" * 80)
    print(
        "Mean VALIDATION eta^2:                    "
        f"{validation_eta['mean']:.6f}"
    )
    print(
        "VALIDATION eta^2 range:                   "
        f"{validation_eta['min']:.6f} - "
        f"{validation_eta['max']:.6f}"
    )
    print(
        "Mean cross-seed VALIDATION Spearman "
        "(mean profiles): "
        f"{validation_mean_rho if validation_mean_rho is not None else 'NA'}"
    )
    print(
        "Mean cross-seed VALIDATION Spearman "
        "(median profiles): "
        f"{validation_median_rho if validation_median_rho is not None else 'NA'}"
    )
    print(
        "Mean TRAIN->VALIDATION profile Spearman:  "
        f"{cross_seed['train_validation_generalization']['mean_rho']}"
    )
    print()
    print("VALIDATION directional consistency:")
    ...
    for cluster in range(EXPECTED_SSL_K):
        ...
        print(
            f"  cluster {cluster}: "
            f"{info['modal_direction']:<8} "
            f"consistency={info['modal_consistency_fraction']:.2f} "
            f"(+ {counts['positive']}, "
            f"- {counts['negative']}, "
