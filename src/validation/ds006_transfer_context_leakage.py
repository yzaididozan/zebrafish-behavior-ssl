#!/usr/bin/env python3
"""Recording/context leakage analysis for DS-006 transfer clusters.

Purpose
-------
Test whether frozen DS-006 transfer clusters are disproportionately determined
by recording/session or experimental-context metadata rather than behavior.
...
These correspond to the replication threats explicitly defined for DS-006:
recording identity, experiment family, well position, and experimental condition.

TRAIN and VALIDATION only. TEST is never loaded.

For every field, seed, and partition the script computes:
- NMI
- AMI
- Cramer's V
- maximum single-category fraction per cluster
- top-3 category fraction per cluster
- effective number of categories per cluster
- per-category normalized cluster entropy
- per-category dominant-cluster fraction
...
Outputs
-------
data/processed/DS-006/transfer_context_leakage/
    seed11/
        train_context_metrics.json
        validation_context_metrics.json
    ...
    aggregate_summary.json
    DS006_TRANSFER_CONTEXT_LEAKAGE_SHA256SUMS
"""
...
OUTPUT_DIR = REPO_ROOT / "data" / "processed" / "DS-006" / "transfer_context_leakage"

SEEDS = (11, 23, 37, 51, 79)
PARTITIONS = ("train", "validation")
FIELDS = ("recording_id", "family", "well", "condition_code", "condition_label")
...
def load_metadata(path: Path, partition: str) -> Dict[str, np.ndarray]:
    ...
    required = {"bout_id", "partition", *FIELDS}
...
def analyze_field(values: np.ndarray, labels: np.ndarray) -> Dict[str, Any]:
    encoded, categories = encode(values)
    table, categories2 = contingency(values, labels)
    if categories != categories2:
        raise RuntimeError("Category encoding mismatch.")

    per_cluster: Dict[str, Any] = {}
    max_single: List[float] = []
    top3: List[float] = []
    ...
    for cluster in range(K):
        ...
        p = counts / total
        p_nonzero = p[p > 0]
        e = float(1.0 / np.sum(p_nonzero ** 2))
        m = float(np.max(p))
        t3 = float(np.sum(np.sort(p)[-3:]))
        max_single.append(m)
        top3.append(t3)
        effective.append(e)
        per_cluster[str(cluster)] = {
            "bout_count": int(total),
            "categories_with_bouts": int(np.sum(counts > 0)),
            "max_single_category_fraction": m,
            "top3_category_fraction": t3,
            "effective_categories": e,
            "dominant_category": categories[int(np.argmax(p))],
        }
    ...
    return {
        ...
        "cluster_concentration_summary": {
            "mean_max_single_category_fraction": float(np.mean(max_single)),
            "max_max_single_category_fraction": float(np.max(max_single)),
            "mean_top3_category_fraction": float(np.mean(top3)),
            "max_top3_category_fraction": float(np.max(top3)),
            "mean_effective_categories_per_cluster": float(np.mean(effective)),
            "min_effective_categories_per_cluster": float(np.min(effective)),
        },
...
def main() -> None:
    ...
    print("=" * 80)
    print("DS-006 TRANSFER RECORDING/CONTEXT LEAKAGE")
    print("=" * 80)
    print(f"Seeds:            {list(SEEDS)}")
    print(f"Metadata fields:  {', '.join(FIELDS)}")
    ...
    for seed in SEEDS:
        ...
        for partition in PARTITIONS:
            ...
            for field in FIELDS:
                result = analyze_field(meta[field], labels)
                partition_result["fields"][field] = result
                a = result["association"]
                c = result["cluster_concentration_summary"]
                d = result["category_distribution_summary"]
                ...
                print(f"    Mean category cluster entropy:    {d['mean_normalized_cluster_entropy']:.6f}")
                print(f"    Mean dominant cluster fraction:   {d['mean_dominant_cluster_fraction']:.6f}")
                print(f"    Mean max category share/cluster:  {c['mean_max_single_category_fraction']:.6f}")
                print(f"    Mean effective categories/cluster:{c['mean_effective_categories_per_cluster']:.2f}")

            results[seed][partition] = partition_result
            out = seed_dir / f"{partition}_context_metrics.json"
            atomic_json(out, partition_result)
            written.append(out)
    ...
    aggregate: Dict[str, Any] = {
        "dataset_id": "DS-006",
        "analysis": "transfer_recording_context_leakage",
        "seeds": list(SEEDS),
        "fields": list(FIELDS),
        "test_partition_used": False,
        "metrics": {},
    ...
    for field in FIELDS:
        ...
        for partition in PARTITIONS:
            aggregate["metrics"][field][partition] = {
                "nmi": summarize([
                    results[s][partition]["fields"][field]["association"]["normalized_mutual_information"]
                    for s in SEEDS
                ]),
                "ami": summarize([
                    results[s][partition]["fields"][field]["association"]["adjusted_mutual_information"]
                    for s in SEEDS
                ]),
                "cramers_v": summarize([
                    results[s][partition]["fields"][field]["association"]["cramers_v"]
                    for s in SEEDS
                ]),
                "mean_normalized_cluster_entropy": summarize([
                    results[s][partition]["fields"][field]["category_distribution_summary"]["mean_normalized_cluster_entropy"]
                    for s in SEEDS
                ]),
                "mean_max_single_category_fraction": summarize([
                    results[s][partition]["fields"][field]["cluster_concentration_summary"]["mean_max_single_category_fraction"]
                    for s in SEEDS
                ]),
                "mean_effective_categories_per_cluster": summarize([
                    results[s][partition]["fields"][field]["cluster_concentration_summary"]["mean_effective_categories_per_cluster"]
                    for s in SEEDS
                ]),
            }
    ...
    checksum = output_dir / "DS006_TRANSFER_CONTEXT_LEAKAGE_SHA256SUMS"
    checksum.write_text(
        "".join(
    ...
    print("=" * 80)
    print("DS-006 CONTEXT-LEAKAGE SUMMARY")
    print("=" * 80)
    for field in FIELDS:
        print(field.upper())
        for partition in PARTITIONS:
            m = aggregate["metrics"][field][partition]
            print(
                f"  {partition.upper():<10} "
                f"NMI={m['nmi']['mean']:.6f}  "
                f"AMI={m['ami']['mean']:.6f}  "
                f"Cramer's V={m['cramers_v']['mean']:.6f}  "
                f"entropy={m['mean_normalized_cluster_entropy']['mean']:.6f}"
            )
    print("TEST partition used: NO")
